...
class LearningCurve():
    ...
    def create_split_dcts(self):
        ...
        # Generate splits on the fly if pre-computed splits were passed
            ...
            if isinstance(self.n_splits, int):
                ...
                idx_vec = np.arange(self.X.shape[0])
                te_size = 0.1
                tr_id, te_id = train_test_split(idx_vec, test_size=te_size, random_state=None)
                vl_size = te_size / (1 - te_size)
                tr_id, vl_id = train_test_split(tr_id, test_size=vl_size, random_state=None)
    ...
    def trn_learning_curve(self,
        ...
        lc_scores = scores_to_df(te_scores_all)
